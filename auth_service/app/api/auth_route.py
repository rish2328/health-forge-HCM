...
async def auth_register ( db: DB_Dependencies, auth_req: RegisterRequest ):
    auth = UserService.auth_register_service ( db, auth_req )
...
async def auth_login ( db: DB_Dependencies, auth_req: LoginRequest ):
    token = UserService.auth_login_request ( db, auth_req.email, auth_req.password )
    return success ( "User logged in successfull", { "access_token": token, "token_type": "Bearer"} )
